...
class BasicBlock:
    ...
    def _gen_asm(self):
        ...
        if self.arch == "x86_64":
            ...
            raise RuntimeError("UNKNOWN ARCH: {}".format(self.arch))
    ...
    def _is_bb_jump(cs_instr, arch):
        ...
        if CS_GRP_JUMP in cs_instr.groups:
            ...
            operand = cs_instr.operands[0]
            base    = operand.mem.base
            val     = operand.mem.segment
            target  = (base << 32) + val 
    ...
    def taint(self, tainted, resolved=None):
        ...
        for i, stmt in enumerate(self.bb.irsb.statements):
            ...
            if isinstance(stmt, pyvex.stmt.Put):
                ...
                try:
                    out_vars.add(LiveVariables.base_reg_name(out_reg_name, self.bb.arch))
                except Exception as e:
                    self.logger.error(e)
                    continue
            ...
            if isinstance(stmt, pyvex.stmt.CAS):
                if hex(stmt.oldHi) != "0xffffffff":
                    #taint oldHi variable, e.g. 5
                    out_vars.add("t{}".format(stmt.oldHi))
            ...
            for expr in stmt.expressions:
                if expr.tag == 'Iex_Get':
                    ...
                    try:
                        in_vars.add(LiveVariables.base_reg_name(reg_name, self.bb.arch))
                    except Exception as e:
                        self.logger.error(e)
                        continue
...
class LiveVariables(BasicBlock):
        ...
        for i, stmt in enumerate(self.bb.irsb.statements):
            ...
            if isinstance(stmt, pyvex.stmt.Put):
                out_reg_name = self.bb.irsb.arch.translate_register_name(stmt.offset,
                    stmt.data.result_size(self.bb.irsb.tyenv) // 8)
                try:
                    base_reg_name = "reg_bb{}_{}_{}".format(self.bb.vaddr, i, LiveVariables.base_reg_name(out_reg_name, self.bb.arch))
            ...
            for expr in stmt.expressions:
                if expr.tag == 'Iex_Get':
                    reg_name = self.bb.irsb.arch.translate_register_name(expr.offset, expr.result_size(self.bb.irsb.tyenv) // 8)
                    try:
                        base_reg_name = "reg_bb{}_{}_{}".format(self.bb.vaddr, i, LiveVariables.base_reg_name(reg_name, self.bb.arch))
                        reg_vars.add(base_reg_name)
                    except Exception as e:
                        self.logger.error(e)
                        continue
    ...
    @staticmethod
    def base_reg_name(reg_name, arch):
        if arch == 'x86-64' or arch == 'x86_64':
            for reg in ArchAMD64.register_list:
                if reg.name == reg_name:
                    return reg.name
            ...
            raise RuntimeError("Unsupported ISA: {}".format(arch))
    ...
    def data_dependencies(self):
        ...
        for i, stmt in enumerate(self.bb.irsb.statements):
            ...
            if isinstance(stmt, pyvex.stmt.WrTmp):
                ...
                try:
                    vex_var     = "reg_bb{}_{}_{}".format(self.bb.vaddr, i, LiveVariables.base_reg_name(reg_name, self.bb.arch))
                except Exception as e:
                    self.logger.error(e)
                    continue
            else:
    ...
    def live_memory_dependencies(self, index=0):
        ...
        #self.logger.debug("Live memory reads from index {} : {}".format(index, tmp_lv))
    ...
    def memory_dependencies(self, index=0):
        ...
        #self.logger.debug("memory reads from index {} : {}".format(index,  tmp_lv))
        ...
        for stmt in self.bb.irsb.statements[index:][::-1]:
            ...
            if isinstance(stmt, pyvex.stmt.Put):
                out_reg_name = self.bb.irsb.arch.translate_register_name(stmt.offset,
                        stmt.data.result_size(self.bb.irsb.tyenv) // 8)
                try:
                    base_reg = LiveVariables.base_reg_name(out_reg_name, self.bb.arch)
            ...
            for expr in stmt.expressions:
                if expr.tag == 'Iex_Get':
                    reg_name = self.bb.irsb.arch.translate_register_name(expr.offset, expr.result_size(self.bb.irsb.tyenv) // 8)
                    try:
                        base_reg = LiveVariables.base_reg_name(reg_name, self.bb.arch)
