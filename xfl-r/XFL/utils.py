...
def chunks_of_size(l, n):
        ...
        #check if generator type, call sub function that doesn't expand full set
        if isinstance(l, types.GeneratorType):
            yield from _gen_chunks_of_size(l, n)
            return
...
def is_scipy_sparse(np_obj):
    """
        Checks if object is Scipy Sparse object
        :return: Bool
    """
    return sp.sparse.issparse(np_obj)

def is_sparse_csr_or_csc(np_obj):
    """
        Checks if obj is a sparse CSC or CSR matrix
        :return:  Bool
    """
    return isinstance(np_obj, sp.sparse.csr.csr_matrix) or isinstance(np_obj, sp.sparse.csc.csc_matrix)
...
def np_force_zero_or_one(np_obj):
    r, c = np_obj.nonzero()
    # set all non-zero elements to 1.0
    np_obj[0, c] = 1.0
    return np_obj
